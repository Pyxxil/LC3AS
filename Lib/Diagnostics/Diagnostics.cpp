...
Diagnostics::Diagnostic::Diagnostic(Diagnostics::FileContext file,
    ...
    : message(std::move(t_message)), d_type(t_type), d(Config::is_set(Config::WARN_AS_ERROR) ? ERROR: diagnostic)
    , context(), info(std::move(file))
{}

Diagnostics::Diagnostic::Diagnostic(Diagnostics::Diagnostic &other)
    : message(other.message), d_type(other.d_type), d(other.d), context(std::move(other.context)), info(other.info)
{}

...
Ldi::Ldi(std::string &instruction,
...
int32_t Ldi::assemble(std::vector<std::shared_ptr<Token>> &tokens,
...
uint16_t Ldi::guess_memory_size(std::vector<std::shared_ptr<Token>> &tokens) const
{
    (void) tokens;
    return static_cast<uint16_t>(is_valid);
}

std::string Ldi::disassemble(uint16_t &program_counter,

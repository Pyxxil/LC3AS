...
int32_t
Add::assemble(std::vector<std::shared_ptr<Token>>& tokens,
              ...
              uint16_t program_counter)
{
  ...
  assembled.emplace_back(static_cast<uint16_t>(
    0x1000 | (std::static_pointer_cast<Register>(tokens.at(1))->reg << 9) |
...
bool
Add::valid_arguments(std::vector<std::shared_ptr<Token>>& tokens)
{
  if (tokens.size() != 4) {
  ...
  return !(tokens.at(1)->is_valid && tokens.at(2)->is_valid &&
...
std::string
Add::disassemble(uint16_t& program_counter,

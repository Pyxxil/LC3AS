...
Str::Str(const std::string &instruction,
...
bool Str::valid_arguments(std::vector<std::shared_ptr<Token>> &tokens) {
  ...
  if (tokens[3]->type() != Token::IMMEDIATE) {
    tokens[3]->expected(Expected::IMMEDIATE_VALUE);
    return (is_valid = false);
  }
  ...
  if (std::static_pointer_cast<Immediate>(tokens[3])->value > 31 ||
    ...
    tokens[3]->requires_too_many_bits(6, SIGNED, this,
...
std::string Str::disassemble(uint16_t &program_counter,
                             const std::string &symbol, int width) const {
  const auto value = assembled.front();

#ifdef INCLUDE_ADDONS
  return fmt::format(
      "({0:04X}) {1:04X} {1:016b} ({2: >4d}) {3: <{4}s} STR R{5:d} R{6:d} "
      "#{7:d}\t{8:s}\n",
      program_counter++, value, line, symbol, width, value >> 9 & 0x7,
      value >> 6 & 0x7, (static_cast<int8_t>(value & 0x3F) << 2) >> 2, file);
#else
  return fmt::format(
      "({0:04X}) {1:04X} {1:016b} ({2: >4d}) {3: <{4}s} STR R{5:d} R{6:d} "
      "#{7:d}\n",
      program_counter++, value, line, symbol, width, value >> 9 & 0x7,
      value >> 6 & 0x7, (static_cast<int8_t>(value & 0x3F) << 2) >> 2);
#endif
}

...
Blkw::Blkw(std::string &directive, std::string &directive_uppercase,
    ...
    if (tokens.size() == 3) {
        if (tokens.at(2)->type() == Token::IMMEDIATE) {
            ...
            if (0u == symbols.count(tokens.at(2)->token)) {
                std::static_pointer_cast<Label>(tokens.at(2))
                    ->not_found(symbols);
                return -1;
            }
                              ...
                              const std::string &symbol, int width) const
{
    ...
    for (size_t block = 1; block < assembled.size();
...
Token::token_type Blkw::type() const { return Token::token_type::DIR_BLKW; }

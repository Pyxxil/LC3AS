...
String::String(const std::string &string, const std::string &t_file,
  ...
  for (size_t index = 0; index < string.length(); ++index) {
    if (string[index] == '\\') {
      if (index + 1 < string.length()) {
          ...
          diagnostic.provide_context(
              std::make_unique<Diagnostics::HighlightContext>(
                  Diagnostics::SelectionContext(
                      Diagnostics::FileContext(t_file, line_number,
                                               t_column + index),
        ...
        Diagnostics::Diagnostic diagnostic(
            Diagnostics::FileContext(t_file, line_number, t_column + index),
            "Unended escape sequence. Using '\'", Diagnostics::SYNTAX,
            Diagnostics::WARNING);

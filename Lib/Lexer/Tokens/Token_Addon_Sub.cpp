...
Sub::Sub(std::string &directive, std::string &directive_uppercase,
                      ...
                      uint16_t program_counter)
{
    ...
    if (std::static_pointer_cast<Register>(tokens.at(first_register_index))->reg
        ...
        std::vector<std::shared_ptr<Token>> vec
            = {set_zero, tokens.at(1), tokens.at(first_register_index),
               decimal_zero};
        ...
        vec = {add, tokens.at(1), tokens.at(first_register_index),
        ...
        if (std::static_pointer_cast<Register>(tokens.at(1))->reg
            != std::static_pointer_cast<Register>(
                   tokens.at(second_register_index))
                   ->reg) {
        ...
        diagnostic.provide_context(
            std::make_unique<Diagnostics::HighlightContext>(
                Diagnostics::SelectionContext(
                    Diagnostics::FileContext(file, line,
                                             column + token.length()),
...
Token::token_type Sub::type() const { return Token::ADDON_SUB; }

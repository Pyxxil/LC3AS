...
Lshift::Lshift(std::string &directive, std::string &directive_uppercase,
                         ...
                         uint16_t program_counter)
{
    std::vector<std::shared_ptr<Token>> vec
        = {add, tokens.at(1), tokens.at(1), tokens.at(1)};
...
bool Lshift::valid_arguments(std::vector<std::shared_ptr<Token>> &tokens)
{
    ...
    if (std::static_pointer_cast<Immediate>(tokens.at(2))->value > 15
        ...
        tokens.at(2)->expected(
            "4 bit unsigned value between 1 and 15 (inclusive)");
        return (is_valid = false);
    }
...
Token::token_type Lshift::type() const { return Token::ADDON_LSHIFT; }

...
Trap::Trap(std::string &instruction, std::string &instruction_uppercase,
...
bool Trap::valid_arguments(std::vector<std::shared_ptr<Token>> &tokens)
{
    ...
    if (std::static_pointer_cast<Immediate>(tokens.at(1))->value > 0xFF) {
        tokens.at(1)->requires_too_many_bits(8, UNSIGNED, this,
...
Token::token_type Trap::type() const { return Token::token_type::OP_TRAP; }

...
Register::Register(const std::string &which, const std::string &which_uppercase,
  ...
  if (reg > 7 || reg < 0) {
    Diagnostics::Diagnostic diagnostic(
        Diagnostics::FileContext(file, line, t_column), "Invalid register",
        Diagnostics::SPELLING, Diagnostics::ERROR);

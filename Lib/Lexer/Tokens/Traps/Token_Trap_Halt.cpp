...
int32_t
Halt::assemble(std::vector<std::shared_ptr<Token>>& tokens,
...
bool
Halt::valid_arguments(std::vector<std::shared_ptr<Token>>& tokens)
{
  if (tokens.size() > 1) {
...
std::string
Halt::disassemble(uint16_t& program_counter,

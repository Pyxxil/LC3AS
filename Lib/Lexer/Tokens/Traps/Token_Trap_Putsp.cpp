...
Putsp::Putsp(const std::string& trap,
...
int32_t
Putsp::assemble(std::vector<std::shared_ptr<Token>>& tokens,
...
bool
Putsp::valid_arguments(std::vector<std::shared_ptr<Token>>& tokens)
{
  if (tokens.size() > 1) {
...
std::string
Putsp::disassemble(uint16_t& program_counter,

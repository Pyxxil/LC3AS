...
public:
  Sub(const std::string &directive, const std::string &directive_uppercase,
      const std::string &t_file, size_t line_number, size_t column);

  inline token_type type() const final { return ADDON_SUB; }
  ...
  void invalid_argument_count(size_t provided, size_t expected,

...
Ldi::Ldi(std::string &instruction, std::string &instruction_uppercase, int line_number)
        : Instruction(instruction, instruction_uppercase, line_number)
{

}

std::int32_t Ldi::assemble(std::vector<std::shared_ptr<Token>> &tokens, const Assembler &assembler)
{
        if (!is_valid) {
                return 0;
        }
...
std::string Ldi::disassemble(std::vector<std::shared_ptr<Token>> &tokens,
                             ...
                             const Assembler &assembler) const
{
        ...
        stream
                ...
                // Instruction itself
                << " LDI " << tokens.at(1)->token_uppercase << ' ';

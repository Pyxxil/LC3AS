...
Sti::Sti(std::string &instruction, std::string &instruction_uppercase, int line_number)
        : Instruction(instruction, instruction_uppercase, line_number)
{

}

int32_t Sti::assemble(std::vector<std::shared_ptr<Token>> &tokens, const Assembler &assembler)
{
        if (!is_valid) {
                return 0;
        }
                             ...
                             const Assembler &assembler) const
{
        ...
        stream
                ...
                // Instruction itself
                << " STI " << tokens.at(1)->token_uppercase << ' ';

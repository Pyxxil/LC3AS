...
Lea::Lea(std::string &instruction, std::string &instruction_uppercase, std::string &t_file, int line_number)
        : Instruction(instruction, instruction_uppercase, t_file, line_number)
{

}

std::int32_t Lea::assemble(std::vector<std::shared_ptr<Token>> &tokens,
...
std::string Lea::disassemble(std::uint16_t &program_counter,
                             ...
                             int width) const
{
        ...
        stream
                ...
                // Instruction itself
                << " LEA R" << ((assembled.front() & 0x0E00) >> 9 & 7 - 0x30) << ' ';
